...
@app.callback(Output(component_id='success-pie-chart', component_property='figure'),
              Input(component_id='launchsite_id', component_property='value'))
def get_pie_chart(entered_site):
    df = spacex_df[['Launch Site', 'class']]
    if entered_site == 'ALL':
        filtered_df = df.groupby('Launch Site',as_index=False)['class'].sum().reset_index()
        fig = px.pie(filtered_df, values='class', names='Launch Site', title="Pie Chart")
        return fig
    else:
        # return the outcomes piechart for a selected site
        filtered_df = df[df['Launch Site']==entered_site].groupby('class').agg(count_rate=('class', 'count')).reset_index()
        fig = px.pie(filtered_df, values='count_rate', names='class', title="Pie Chart")
        return fig
...
def get_pie_chart(entered_site,entered_payload):
    ...
    if entered_site == 'ALL':
        fig1 = px.scatter(df, x="Payload Mass (kg)", y="class", color="Booster Version Category")
        return fig1
    else:
        # return the outcomes piechart for a selected site
